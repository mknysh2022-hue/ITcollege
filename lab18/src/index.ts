...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 2. ДОДАНО: Більш строга перевірка на null для всіх DOM-елементів
    ...
    async function fetchTodos() {
        ...
        try {
            ...
            list.innerHTML = "";
            // 3. ПОРАДА: Можна додати фільтрацію за USER_ID, якщо сервер повертає чужі задачі
            const myTasks = data.filter(task => task.userId === USER_ID);
            myTasks.forEach((task) => addTaskToDOM(task, true));
        } catch (error) {
            console.error(error);
    ...
    function addTaskToDOM(task: Todo, append = false) {
        ...
        // 4. ВИПРАВЛЕНО: Безпечне присвоєння ID через dataset
        if (task.id) el.dataset.id = task.id;
    ...
    form.addEventListener("submit", async (event) => {
        ...
        const taskData: Todo = {
            ...
            // Якщо редагуємо, не змінюємо дату створення
    ...
    list.addEventListener("click", async (event) => {
        ...
        if (btn.classList.contains("btn--edit-action")) {
            ...
            // 5. ВИПРАВЛЕНО: Більш безпечне отримання елементів форми
            const titleInput = form.elements.namedItem("title") as HTMLInputElement;
            const descInput = form.elements.namedItem("description") as HTMLTextAreaElement;
            const tagSelect = form.elements.namedItem("tag") as HTMLSelectElement;
            const deadlineInput = form.elements.namedItem("deadline") as HTMLInputElement;

            titleInput.value = (card.querySelector(".task-card__title") as HTMLElement).textContent || "";
            descInput.value = (card.querySelector(".task-card__desc") as HTMLElement).textContent || "";
            tagSelect.value = (card.querySelector(".task-card__tag") as HTMLElement).textContent || "";
            
            let deadlineText = (card.querySelector(".task-card__deadline") as HTMLElement).textContent;
            deadlineInput.value = deadlineText === "Без дати" ? "" : (deadlineText || "");
