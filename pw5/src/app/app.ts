...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [FormsModule], // Обов'язково для роботи [(ngModel)]
  templateUrl: './app.html',
  styleUrls: ['./app.scss'],
})
export class App implements OnInit {
  USER_ID = 12; // Твій номер за журналом 
  ...
  fetchTodos() {
    ...
    this.todoService.getAll().subscribe({
      next: (data) => {
        // Фільтруємо, щоб бачити тільки свої задачі, та розвертаємо список [cite: 852]
  ...
  editTask(task: Todo) {
    ...
    this.formData = { ...task }; // Копіюємо дані в форму [cite: 902]
    this.cdr.detectChanges();
  }
