...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchTodos() {
    ...
    try {
      ...
      list.innerHTML = "";
      // Фільтруємо задачі, щоб показувати тільки твої (userId: 12)
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (isEditing && currentEditId) {
      // === ЗАВДАННЯ 2: Оновлення запису (PUT) ===
      try {
        ...
        if (!response.ok) throw new Error("Помилка сервера при оновленні");
  ...
  list.addEventListener("click", async (event) => {
    ...
    if (btn.classList.contains("btn--delete")) {
      if (confirm("Видалити цю задачу з сервера?")) {
        // === ЗАВДАННЯ 1: Видалення запису (DELETE) ===
        try {
